...
router.post('/add_new_company',upload.single('logo'), function(req, res, next) {
  ...
  pool.query("insert into company ( companyname, ownername, emailaddress, mobilenumber, state, city, address, logo, password, status, createdat, updateat, createdby)values(?,?,?,?,?,?,?,?,?,?,?,?,?)",[req.body.companyname,req.body.ownername,req.body.emailaddress,req.body.mobilenumber,req.body.state,req.body.city,req.body.address,req.file.originalname,req.body.password,req.body.status,req.body.createdat,req.body.updateat,req.body.createdby],function(error,result){
    if(error)
    {
...
router.post('/edit_company_data', function(req, res, next) {
  ...
  pool.query("update company set companyname=?, ownername=?, emailaddress=?, mobilenumber=?, state=?, city=?, address=?, status=?, updateat=?, createdby=?  where companyid=?" ,[req.body.companyname,req.body.ownername,req.body.emailaddress,req.body.mobilenumber,req.body.state,req.body.city,req.body.address,req.body.status,req.body.updateat,req.body.createdby,req.body.companyid],function(error,result){
    if(error)
    {
        console.log(error)
    res.status(500).json({status:false,message:'Server Error'})
...
router.post('/delete_company_data',upload.single('logo') ,function(req, res, next) {
